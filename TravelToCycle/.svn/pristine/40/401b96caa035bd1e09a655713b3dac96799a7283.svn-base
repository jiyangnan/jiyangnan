	...
	public void oper(String time){
		...
		if(CollectionUtils.isEmpty(list)){
			...
			if (myDate1.before(lastDate)) {
				sql = "SELECT e.CHANNEL, SUM(`e`.`CONCERNSCOUNT`) AS `CONCERNSCOUNT`,SUM(`e`.`COLLECTCOUNT`) AS `COLLECTCOUNT`, SUM(`e`.`FORWARDCOUNT`) AS `FORWARDCOUNT`, SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_twitter_data_paste e WHERE DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD('"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
			}else {
				sql = "SELECT e.CHANNEL, SUM(`e`.`CONCERNSCOUNT`) AS `CONCERNSCOUNT`,SUM(`e`.`COLLECTCOUNT`) AS `COLLECTCOUNT`, SUM(`e`.`FORWARDCOUNT`) AS `FORWARDCOUNT`, SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_twitter_data_"+paramYearUS+"_"+paramWeekUS+" e WHERE DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD('"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
			}
			...
			DBUtil.addListMap(mapList, "wk_twitter_data_channel_date_day");

		}else{
			System.out.println("wk_twitter_data_channel_date_day已有数据！！！");
		}
