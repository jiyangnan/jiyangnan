...
public class TwitterQueryIntoTempMonth {
	...
	public void oper(String time){
		...
		if(CollectionUtils.isEmpty(list)){
			//生成channel中间表
			String sql = "SELECT e.CHANNEL, e.MONTHNUM , CAST(SUM(`e`.`CONCERNSCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `CONCERNSCOUNT`,CAST(SUM(`e`.`COLLECTCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `COLLECTCOUNT`, CAST(SUM(`e`.`FORWARDCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `FORWARDCOUNT`, CAST(SUM(`e`.`REVIEWCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `REVIEWCOUNT`, CAST(COUNT(`e`.`CITEMS`)/ "+totalDay+" as decimal(18,0)) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_twitter_data_channel_date_day e WHERE DATETIME >( DATE_ADD('"+finalDay+"',INTERVAL -"+totalDay+" DAY) ) AND DATETIME< '"+finalDay+"' GROUP BY CHANNEL";
