	...
	public void oper(String time){
		...
		if(CollectionUtils.isEmpty(list)){
			...
			if (myDate1.before(lastDate)) {
				sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`, SUM(`e`.`SHARECOUNT`) AS `SHARECOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_ggplus_data_paste e WHERE DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD( '"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
			}else {
				sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`, SUM(`e`.`SHARECOUNT`) AS `SHARECOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_ggplus_data_"+paramYearUS+"_"+paramWeekUS+" e WHERE DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD('"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
			}
			...
			DBUtil.addListMap(mapList, "wk_ggplus_data_channel_date_day");

		}else{
			System.out.println("wk_ggplus_data_channel_date_day表数据已生成！！！");
		}
