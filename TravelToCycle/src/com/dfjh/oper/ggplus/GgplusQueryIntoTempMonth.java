...
public class GgplusQueryIntoTempMonth {
	...
	public void oper(String time){
		...
		String sqlBlog = "SELECT CHANNEL from wk_ggplus_data_channel_date_month where MONTHNUM = '"+lastMonthStr+"'";
		...
		if(CollectionUtils.isEmpty(list)){
			//生成channel中间表
			String sql = "SELECT e.CHANNEL,e.MONTHNUM, CAST(SUM(`e`.`VIEWCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `VIEWCOUNT`,CAST(SUM(`e`.`GOODCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `GOODCOUNT`,CAST(SUM(`e`.`BADCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `BADCOUNT`, CAST(SUM(`e`.`REVIEWCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `REVIEWCOUNT`, CAST(SUM(`e`.`SHARECOUNT`)/ "+totalDay+" as decimal(18,0)) AS `SHARECOUNT`, CAST(COUNT(`e`.`CITEMS`)/ "+totalDay+" as decimal(18,0)) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_ggplus_data_channel_date_day e WHERE DATETIME >( DATE_ADD('"+finalDay+"',INTERVAL -"+totalDay+" DAY) ) AND DATETIME< '"+finalDay+"' GROUP BY CHANNEL";
			...
			DBUtil.addListMap(mapList, "wk_ggplus_data_channel_date_month");
		}else{
			System.out.println("wk_ggplus_data_channel_date_month 表中对应此月份数据已经生成！");
		}
...
//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
//		Date date = new Date();
//		new GgplusQueryIntoTempMonth().oper("2015-10-09", setNum);
//	}
}
