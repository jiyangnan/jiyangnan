	...
	public void oper(String time){
		...
		for(int i=0;i<channelList.size();i++){
			...
			if(CollectionUtils.isEmpty(list)){
				...
				"e.UPLOADTIME  FROM  wk_facebook_data_"+year+"_"+month+"_"+day+" e where CHANNEL=\""+channelName+"\" and TITLE like \"%"+channelName+"%\" GROUP BY CHANNEL";
				...
				System.out.println("插入wk_facebook_data_channel_date_total表大小为："+mapList.size());

			}else{
				System.out.println("wk_facebook_data_channel_date_total已有今日数据！");
			}
		}
