		...
		public void oper(String time){
			...
			if(CollectionUtils.isEmpty(list)){
				...
				String sql = "SELECT e.CHANNEL, e.WEEKNUM, " +
						"CAST(SUM(`e`.`CONCERNSCOUNT`)/7 as decimal(18,0)) AS `CONCERNSCOUNT`," +
						"CAST(SUM(`e`.`COLLECTCOUNT`)/7 as decimal(18,0)) AS `COLLECTCOUNT`, " +
						"CAST(SUM(`e`.`FORWARDCOUNT`)/7 as decimal(18,0)) AS `FORWARDCOUNT`, " +
						"CAST(SUM(`e`.`REVIEWCOUNT`)/7 as decimal(18,0)) AS `REVIEWCOUNT`," +
						"CAST(COUNT(`e`.`CITEMS`)/7 as decimal(18,0)) AS `CITEMS`,e.DATETIME ," +
						"e.UPLOADTIME  FROM  wk_twitter_data_channel_date_day_new_news e WHERE " +
...
//							DBUtil.addListMap(mapList7, "wk_twitter_data_channel_date_week");
							...
							System.out.println("wk_twitter_data_channel_date_week 表中对应此周数据已经生成！");
