	...
	public void oper(String time){
		...
		if(CollectionUtils.isEmpty(list)){
			...
			String sql = "SELECT e.CHANNEL,e.MONTHNUM, " +
					...
					"CAST(SUM(`e`.`GOODCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `GOODCOUNT`," +
					"CAST(SUM(`e`.`BADCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `BADCOUNT`," +
					"CAST(SUM(`e`.`REVIEWCOUNT`)/ "+totalDay+" as decimal(18,0)) AS `REVIEWCOUNT`, " +
					"CAST(SUM(`e`.`SHARECOUNT`)/ "+totalDay+" as decimal(18,0)) AS `SHARECOUNT`, " +
					"CAST(COUNT(`e`.`CITEMS`)/ "+totalDay+" as decimal(18,0)) AS `CITEMS`," +
					"e.DATETIME ,e.UPLOADTIME  FROM  wk_facebook_data_channel_date_day_new_news e " +
					"WHERE DATETIME >( DATE_ADD('"+finalDay+"',INTERVAL -"+totalDay+" DAY) ) AND DATETIME< '"+finalDay+"' GROUP BY CHANNEL";
