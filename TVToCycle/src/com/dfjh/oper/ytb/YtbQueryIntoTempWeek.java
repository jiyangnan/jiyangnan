...
public class YtbQueryIntoTempWeek {
	...
	public void oper(String time){
		...
		String sqlBlog = "SELECT CHANNEL from wk_ytb_data_channel_date_week where WEEKNUM = '"+WeekNow+"'";
		...
		if(CollectionUtils.isEmpty(list)){
			...
			String sql = "SELECT e.CHANNEL, e.WEEKNUM, " +
					...
					"e.UPLOADTIME  FROM  wk_ytb_data_channel_date_day_new_news e " +
					"WHERE DATETIME IN('"+monday+"','"+tuesday+"','"+wensday+"','"+thursday+"','"+friday+"','"+saturday+"','"+sunday+"') " +
					"AND UPLOADTIME>= DATE_ADD('"+monday+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
			...
			DBUtil.addListMap(mapList, "wk_ytb_data_channel_date_week");



		}else{
			System.out.println("wk_ytb_data_channel_date_week 表中对应此周数据已经生成！");
...
//				Calendar cal = Calendar.getInstance();
//				new YtbQueryIntoTempWeek().oper("2015-09-14");
//			}
}
