		...
		if(CollectionUtils.isEmpty(list)){
			...
			for(int i=0;i<channelList.size();i++){
				...
				if(myDate1.before(lastDate)) {
					...
					sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_ytb_data_paste_channel e WHERE CHANNEL='"+channelName+"' and TITLE like '%"+channelName+"%' and DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD( '"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
					...
					//若以上两个条件均不满足，照常执行
					sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_ytb_data_"+paramYearUS+"_"+paramWeekUS+" e WHERE CHANNEL='"+channelName+"' and TITLE like '%"+channelName+"%' and DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD('"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
				}
				...
				DBUtil.addListMap(mapList, "wk_ytb_data_channel_date_day_new_news");

			}
		}else{
			System.out.println("wk_ytb_data_channel_date_new_news 数据已生成！");
		}
