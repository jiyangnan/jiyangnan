...
public class GgplusQueryIntoTempDay {
	...
	public void oper(String time){
		...
		String sqlBlog = "SELECT CHANNEL from wk_ggplus_data_channel_date_day where DATETIME = '"+time+"'";
		...
		if(CollectionUtils.isEmpty(list)){
			...
			for(int i=0;i<channelList.size();i++){
				...
				if (myDate1.before(lastDate)) {
					sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`, SUM(`e`.`SHARECOUNT`) AS `SHARECOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_ggplus_data_paste e where CHANNEL='"+channelName+"' and TITLE like '%"+channelName+"%' and   DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD( '"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
				}else {
					sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`, SUM(`e`.`SHARECOUNT`) AS `SHARECOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_ggplus_data_"+paramYearUS+"_"+paramWeekUS+" e where CHANNEL='"+channelName+"' and TITLE like '%"+channelName+"%' and DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD('"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
				}
				...
				DBUtil.addListMap(mapList, "wk_ggplus_data_channel_date_day_new_news");

			}
		}else{
			System.out.println("wk_ggplus_data_channel_date_day_new_news表数据已生成！！！");
		}
	...
	//		Calendar cal = Calendar.getInstance();
	//		new GgplusQueryIntoTempDay().oper("2015-09-09", setNum);
	//	}
}
