...
public class GgplusQueryIntoTempWeek {
	...
	public void oper(String time){
		...
		String sqlBlog = "SELECT CHANNEL from wk_ggplus_data_channel_date_week where WEEKNUM = '"+WeekNow+"'";
		...
		if(CollectionUtils.isEmpty(list)){
			//生成channel中间表
			String sql = "SELECT e.CHANNEL,  e.WEEKNUM, " +
					"CAST(SUM(`e`.`VIEWCOUNT`)/7 as decimal(18,0)) AS `VIEWCOUNT`," +
					"CAST(SUM(`e`.`GOODCOUNT`)/7 as decimal(18,0)) AS `GOODCOUNT`, " +
					"CAST(SUM(`e`.`BADCOUNT`)/7 as decimal(18,0)) AS `BADCOUNT`," +
					"CAST(SUM(`e`.`REVIEWCOUNT`)/7 as decimal(18,0)) AS `REVIEWCOUNT`, " +
					"CAST(SUM(`e`.`SHARECOUNT`)/7 as decimal(18,0)) AS `SHARECOUNT`," +
					"CAST(COUNT(`e`.`CITEMS`)/7 as decimal(18,0)) AS `CITEMS`,e.DATETIME ," +
					"e.UPLOADTIME  FROM  wk_ggplus_data_channel_date_day_new_news e WHERE " +
					"DATETIME IN('"+monday+"','"+tuesday+"','"+wensday+"','"+thursday+"','"+friday+"','"+saturday+"','"+sunday+"') AND UPLOADTIME>= DATE_ADD('"+monday+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
			...
			DBUtil.addListMap(mapList, "wk_ggplus_data_channel_date_week");

		}else{
			System.out.println("wk_ggplus_data_channel_date_week 表中对应此周数据已经生成！");
	...
	//		Calendar cal = Calendar.getInstance();
	//		new GgplusQueryIntoTempWeek().oper("2015-09-14", setNum);
	//	}
}
