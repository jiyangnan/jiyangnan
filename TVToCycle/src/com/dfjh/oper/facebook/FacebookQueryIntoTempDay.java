		...
		if(CollectionUtils.isEmpty(list)){
			...
			for(int i=0;i<channelList.size();i++){
				...
				if (myDate1.before(lastDate)) {
					sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`, SUM(`e`.`SHARECOUNT`) AS `SHARECOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_facebook_data_paste e WHERE CHANNEL='"+channelName+"' and TITLE like '%"+channelName+"%' and DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD('"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
				}else{
					sql = "SELECT e.CHANNEL, SUM(`e`.`VIEWCOUNT`) AS `VIEWCOUNT`,SUM(`e`.`GOODCOUNT`) AS `GOODCOUNT`,SUM(`e`.`BADCOUNT`) AS `BADCOUNT`,SUM(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`, SUM(`e`.`SHARECOUNT`) AS `SHARECOUNT`,COUNT(`e`.`URL`) AS `CITEMS`,e.DATETIME ,e.UPLOADTIME  FROM  wk_facebook_data_"+paramYearUS+"_"+paramWeekUS+" e where CHANNEL='"+channelName+"' and TITLE like '%"+channelName+"%' and  DATETIME='"+time+"' AND UPLOADTIME>= DATE_ADD('"+time+"',INTERVAL -100 DAY) GROUP BY CHANNEL";
				}
				//生成channel中间表
				...
				DBUtil.addListMap(mapList, "wk_facebook_data_channel_date_day_new_news");

			}
		}else{
			System.out.println("wk_facebook_data_channel_date_day_new_news表数据已生成！！！");
		}
