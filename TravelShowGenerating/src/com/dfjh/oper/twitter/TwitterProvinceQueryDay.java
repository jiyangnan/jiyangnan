 ...
 */
public class TwitterProvinceQueryDay {
	...
	public void operFromTemp(String time,int setNum){
		...
		String channelJudge = "select * from ufeng_twitter_tops_tv_count_day where update_time= '"+time+"'";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			String channelSql = "SELECT a.*,a.id as top_id,  "
				...
				+ "LEFT JOIN ufeng_twitter_tops_class_count_day as b "
			...
			DBUtil.addListMapForTVCount(channelMapList, "ufeng_twitter_tops_tv_count_day");
			System.out.println("插入ufeng_twitter_tops_tv_count_day表大小为："+channelMapList.size());
		}
