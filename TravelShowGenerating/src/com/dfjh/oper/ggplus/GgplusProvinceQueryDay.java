 ...
 */
public class GgplusProvinceQueryDay {
	...
	public void operFromTemp(String time,int setNum){
		...
		String channelJudge = "select * from ufeng_ggplus_tops_tv_count_day where update_time= '"+time+"'";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			String channelSql = "SELECT a.*,a.id as top_id,b.month as month,b.week as week,"
				...
				+ "LEFT JOIN ufeng_ggplus_tops_class_count_day as b "
			...
			DBUtil.addListMapForTVCount(channelMapList, "ufeng_ggplus_tops_tv_count_day");
			System.out.println("插入ufeng_ggplus_tops_tv_count_day表大小为："+channelMapList.size());
		}
