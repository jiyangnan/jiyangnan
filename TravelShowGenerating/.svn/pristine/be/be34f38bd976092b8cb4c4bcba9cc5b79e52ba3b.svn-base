 ...
 */
public class FacebookSiteQueryDay {
	...
	public void operFromTemp(String time, int setNum, String taday,String yesTaday) {
		...
		String channelJudge = "select * from ufeng_facebook_tops_channel_count_day where update_time= '"+taday+"'";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			String channelSql = "SELECT  a.*,a.id as top_id,d.WEEKNUM AS week,"
				...
				+ "wk_facebook_data_channel_date_day as d "
				+ "ON b.channel = d.channel  join  wk_facebook_data_channel_date_day as f on f.datetime='"+ yesTaday+ "'"
			...
			DBUtil.addListMapForClassCount(channelMapList,"ufeng_facebook_tops_channel_count_day");
			System.out.println("插入ufeng_facebook_tops_channel_count_day表大小为："+ channelMapList.size());
		}
