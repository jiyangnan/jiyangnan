 ...
 */
public class YtbSiteQueryMonth {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  month from ufeng_ytb_tops_channel_count_month where month='"+lastMonthStr+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				"b.`LEVEL` AS na "+
				"from  ufeng_tops as a  JOIN  wk_mapcode AS b  ON a.title = b.channel " +
				" join wk_ytb_data_channel_date_month as d " +
				" ON b.channel = d.channel join wk_ytb_data_channel_date_month as f on f.MONTHNUM='"+lastLastMonthStr+"'" +
				" and d.channel= f.channel where d.MONTHNUM='"+lastMonthStr+"'";
				List<Map<String,Object>> channelMapList = null;
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());
				DBUtil.addListMapForClassCount(channelMapList, "ufeng_ytb_tops_channel_count_month");
				System.out.println("插入ufeng_ytb_tops_channel_count_month表大小为："+channelMapList.size());
				System.out.println(channelSql);
			}
	...
	//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	//		Date date = new Date();
	//		new YtbChannelQueryMonth().operFromTemp("2015-09-15", setNum);
	//	}
}
