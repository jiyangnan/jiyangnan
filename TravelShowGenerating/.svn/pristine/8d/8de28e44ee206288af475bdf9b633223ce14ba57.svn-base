 ...
 */
public class GgplusCityQueryMonth {
		...
		try {
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT a.*,a.id as top_id,b.month as month,"
					...
					+ "LEFT JOIN ufeng_ggplus_tops_channel_count_month as b "
					+ "ON a.id = b.pid and b.month ='"+lastMonthStr+"' WHERE a.level='2'  GROUP BY id";;
					List<Map<String,Object>> channelMapList = null;
					channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());
					DBUtil.addListMapForTVCount(channelMapList, "ufeng_ggplus_tops_class_count_month");
	...
	//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	//		Date date = new Date();
	//		new GgplusTVQueryMonth().operFromTemp("2015-09-15", setNum);
	//	}
}
