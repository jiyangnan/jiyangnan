 ...
 */
public class FacebookCityQueryDay {
	...
	public void operFromTemp(String time,int setNum){
		...
		String channelJudge = "select * from ufeng_facebook_tops_class_count_day where update_time= '"+time+"'";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			String channelSql = "SELECT a.*,a.id as top_id,b.month as month,b.week as week,"
				...
				+ "JOIN ufeng_facebook_tops_channel_count_day as b "
			...
			DBUtil.addListMapForTVCount(channelMapList, "ufeng_facebook_tops_class_count_day");
