 ...
 */
public class GgplusSiteQueryMonth {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  month from ufeng_ggplus_tops_channel_count_month where month='"+lastMonthStr+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT  a.*,a.id as top_id, d.MONTHNUM AS month,"
					...
					+ "wk_ggplus_data_channel_date_month as d "
					+ " ON b.channel = d.channel  join  wk_ggplus_data_channel_date_month as f on f.MONTHNUM='"+lastLastMonthStr+ "'"
				...
				DBUtil.addListMapForClassCount(channelMapList, "ufeng_ggplus_tops_channel_count_month");
				System.out.println("插入ufeng_ggplus_tops_channel_count_month表大小为："+channelMapList.size());
			}
