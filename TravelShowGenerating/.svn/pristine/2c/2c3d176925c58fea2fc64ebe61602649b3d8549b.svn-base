...
public class YtbSiteQueryWeek {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  week from ufeng_ytb_tops_channel_count_week where week='"+WeekNow+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				"join wk_ytb_data_channel_date_week as d " +
				" ON b.channel = d.channel  join wk_ytb_data_channel_date_week as f on f.WEEKNUM='"+WeekNow+"'" +
				...
				DBUtil.addListMapForClassCount(channelMapList, "ufeng_ytb_tops_channel_count_week");
				System.out.println("插入ufeng_ytb_tops_channel_count_week表大小为："+channelMapList.size());
