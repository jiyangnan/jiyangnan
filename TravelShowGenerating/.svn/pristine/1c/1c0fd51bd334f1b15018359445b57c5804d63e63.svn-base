 ...
 */
public class FacebookCityQueryMonth {
		...
		try {
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT a.*,a.id as top_id,b.month as month,"
					...
					+ "LEFT JOIN ufeng_facebook_tops_channel_count_month as b "
					+ "ON a.id = b.pid and a.level='2' and b.month='"+lastMonthStr+"' GROUP BY id";
				List<Map<String,Object>> channelMapList = null;
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());
				DBUtil.addListMapForTVCount(channelMapList, "ufeng_facebook_tops_class_count_month");
