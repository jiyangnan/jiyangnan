 ...
 */
public class FacebookCityQueryWeek {
		...
		try {
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT a.*,a.id as top_id,b.week as week,"
					...
					+ "LEFT JOIN ufeng_facebook_tops_channel_count_week as b "
					+ "ON a.id = b.pid  and a.level='2' WHERE b.week ='"+WeekNow+"'  GROUP BY id";
				List<Map<String,Object>> channelMapList = null;
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());
				DBUtil.addListMapForTVCount(channelMapList, "ufeng_facebook_tops_class_count_week");
	...
	//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	//		Date date = new Date();
	//		new FacebookTVQueryWeek().operFromTemp("2015-09-15", setNum);
	//	}
}
