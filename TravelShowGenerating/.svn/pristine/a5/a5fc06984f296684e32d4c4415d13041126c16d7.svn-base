 ...
 */
public class GgplusCityQueryWeek {
		...
		try {
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT a.*,a.id as top_id,b.week as week,"
					...
					+ "LEFT JOIN ufeng_ggplus_tops_channel_count_week as b "
					+ "ON a.id = b.pid and b.week ='"+WeekNow+"' WHERE a.level='2'  GROUP BY id";
				List<Map<String,Object>> channelMapList = null;
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());
				DBUtil.addListMapForTVCount(channelMapList, "ufeng_ggplus_tops_class_count_week");
	...
	//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	//		Date date = new Date();
	//		new GgplusTVQueryWeek().operFromTemp("2015-09-15", setNum);
	//	}
}
