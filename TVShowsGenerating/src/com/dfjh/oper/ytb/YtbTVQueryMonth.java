...
public class YtbTVQueryMonth {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  month from ufeng_ytb_tops_tv_count_month where month='"+lastMonthStr+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT a.*,a.id as top_id, b.month as month, "
						...
						+ "LEFT JOIN ufeng_ytb_tops_class_count_month as b "
						+ "ON a.id = b.pid WHERE a.level='1' and b.month ='"+lastMonthStr+"' GROUP BY id";
				...
				DBUtil.addListMapForTVCount(channelMapList, "ufeng_ytb_tops_tv_count_month");
				System.out.println("插入ufeng_ytb_tops_tv_count_month表大小为："+channelMapList.size());
			}
