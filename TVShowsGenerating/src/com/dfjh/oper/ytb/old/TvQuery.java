...
//		DBHelper.setFileName(propertiesName);
		...
		String sql = "SELECT a.*,a.id as top_id,SUM(b.view_count) as view_count,SUM(b.good_count) as good_count ,SUM(b.bad_count) as bad_count,SUM(b.review_count) as review_count,SUM(b.citems_count) as citems_count,b.update_time FROM ufeng_tops as a LEFT JOIN ufeng_ytb_tops_class_count as b ON a.id = b.pid WHERE a.level='1' and b.update_time ='"+time+"' GROUP BY id";
	
		List<Map<String,Object>> mapList = null;
		try {
			mapList = DBHelper.getQueryRunner().query(sql, new MapListHandler());
			
			System.out.println("插入ufeng_ytb_tops_tv_count表大小为："+mapList.size());
		} catch (SQLException e) {
			e.printStackTrace();
		}
		
		DBUtil.addListMapForTVCount(mapList, "ufeng_ytb_tops_tv_count");
		
		//查询原始数据表中SUBSCRIBE字段值
//		String upSql = "select a.AUTHOR,a.SUBSCRIBE from wk_ytb_data as a JOIN ufeng_ytb_tops_tv_count as b on a.AUTHOR = b.remarks WHERE a.DATETIME = '"+time+"' GROUP BY AUTHOR ORDER BY CAST(a.SUBSCRIBE as SIGNED) DESC";
...
//					String updateSql = "UPDATE ufeng_ytb_tops_tv_count SET subscribe = '"+subscribe+"' where remarks = '"+remarks+"' and update_time = '"+time+"'";
//					try {
