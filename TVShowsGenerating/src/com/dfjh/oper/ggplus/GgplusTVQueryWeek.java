...
public class GgplusTVQueryWeek {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  week from ufeng_ggplus_tops_tv_count_week where week='"+WeekNow+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT a.*,a.id as top_id,b.week as week,"
					...
					+ "LEFT JOIN ufeng_ggplus_tops_class_count_week as b "
					+ "ON a.id = b.pid and b.week ='"+WeekNow+"' WHERE a.level='1'  GROUP BY id";
				List<Map<String,Object>> channelMapList = null;
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());
				DBUtil.addListMapForTVCount(channelMapList, "ufeng_ggplus_tops_tv_count_week");
				System.out.println("插入ufeng_ggplus_tops_tv_count_week表大小为："+channelMapList.size());
			}
