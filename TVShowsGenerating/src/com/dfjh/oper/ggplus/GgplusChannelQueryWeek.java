...
public class GgplusChannelQueryWeek {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  week from ufeng_ggplus_tops_class_count_week where week='"+WeekNow+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT  a.*,a.id as top_id,d.WEEKNUM AS week,"
					...
					+ "ufeng_tops_channel as c on c.id = b.channel_id join wk_ggplus_data_channel_date_week as d "
					+ " on c.code=d.channel  join  wk_ggplus_data_channel_date_week as f on f.WEEKNUM='"+WeekNow+"'"
				...
				DBUtil.addListMapForClassCount(channelMapList, "ufeng_ggplus_tops_class_count_week");
				System.out.println("插入ufeng_ggplus_tops_class_count_week表大小为："+channelMapList.size());
			}
