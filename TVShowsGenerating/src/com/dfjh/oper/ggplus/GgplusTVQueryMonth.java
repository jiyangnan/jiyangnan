...
public class GgplusTVQueryMonth {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  month from ufeng_ggplus_tops_tv_count_month where month='"+lastMonthStr+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT a.*,a.id as top_id,b.month as month,"
					...
					+ "sum(b.citems_count) as citems_count,sum(b.citems_count_interval) as citems_count_interval,"
						...
						+ "LEFT JOIN ufeng_ggplus_tops_class_count_month as b "
						+ "ON a.id = b.pid and b.month ='"+lastMonthStr+"' WHERE a.level='1'  GROUP BY id";;
				List<Map<String,Object>> channelMapList = null;
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());
				System.out.println("插入ufeng_ggplus_tops_tv_count_month表大小为："+channelMapList.size());
				DBUtil.addListMapForTVCount(channelMapList, "ufeng_ggplus_tops_tv_count_month");
				System.out.println("插入ufeng_ggplus_tops_tv_count_month表大小为："+channelMapList.size());
