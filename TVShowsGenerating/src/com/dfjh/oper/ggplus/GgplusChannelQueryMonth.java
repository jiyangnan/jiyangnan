...
public class GgplusChannelQueryMonth {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  month from ufeng_ggplus_tops_class_count_month where month='"+lastMonthStr+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				String channelSql = "SELECT  a.*,a.id as top_id, d.MONTHNUM AS month,"
					...
					+ "ufeng_tops_channel as c on c.id = b.channel_id join wk_ggplus_data_channel_date_month as d "
					+ " on c.code=d.channel  join  wk_ggplus_data_channel_date_month as f on f.MONTHNUM='"+lastLastMonthStr+ "'"
				...
				DBUtil.addListMapForClassCount(channelMapList, "ufeng_ggplus_tops_class_count_month");
				System.out.println("插入ufeng_ggplus_tops_class_count_month表大小为："+channelMapList.size());
			}
	...
	//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	//		Date date = new Date();
	//		new GgplusChannelQueryMonth().operFromTemp("2015-09-15", setNum);
	//	}
}
