...
public class GgplusChannelQueryDay {
	...
	public void operFromTemp(String time, int setNum, String taday,String yesTaday) {
		...
		String channelJudge = "select * from ufeng_ggplus_tops_class_count_day where update_time= '"+taday+"'";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			/*
			 * String channelSql =
			 * "SELECT  a.*,a.id as top_id,d.WEEKNUM AS week, d.MONTHNUM AS month,"
			 * +
			 * "d.VIEWCOUNT AS view_count, d.GOODCOUNT as good_count, d.BADCOUNT AS bad_count ,"
			 * +
			 * "d.SHARECOUNT AS share_count ,d.REVIEWCOUNT as review_count ,d.CITEMS as citems_count,"
			 * +
			 ...
			 * +
			 * "JOIN wk_ggplus_data_channel_date_day as d on c.code=d.channel and d.DATETIME='"
			 * +time+"'";
			 */
			String channelSql = "SELECT  a.*,a.id as top_id,d.WEEKNUM AS week, d.MONTHNUM AS month,"
				...
				+ "ufeng_tops_channel as c on c.id = b.channel_id join wk_ggplus_data_channel_date_day_new_news as d "
				+ " on c.code=d.channel  join  wk_ggplus_data_channel_date_day_new_news as f on f.datetime='"+yesTaday+ "'"
