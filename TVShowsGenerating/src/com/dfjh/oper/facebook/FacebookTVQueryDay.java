...
public class FacebookTVQueryDay {
	...
	public void operFromTemp(String time,int setNum){
		...
		String channelJudge = "select * from ufeng_facebook_tops_tv_count_day where update_time= '"+time+"'";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			String channelSql = "SELECT a.*,a.id as top_id,b.month as month,b.week as week,"
				...
				+ "b.update_time FROM ufeng_tops as a LEFT "
				+ "JOIN ufeng_facebook_tops_class_count_day as b "
				+ "ON a.id = b.pid and b.update_time ='"+time+"' WHERE a.level='1'  GROUP BY id";

			List<Map<String,Object>> channelMapList = null;
			try {
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());

				System.out.println("插入ufeng_facebook_tops_tv_count_day表大小为："+channelMapList.size());
			} catch (SQLException e) {
				e.printStackTrace();
			}

			DBUtil.addListMapForTVCount(channelMapList, "ufeng_facebook_tops_tv_count_day");
			System.out.println("插入ufeng_facebook_tops_tv_count_day表大小为："+channelMapList.size());
		}
	...
	//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	//		Date date = new Date();
	//		new FacebookTVQueryDay().operFromTemp("2015-09-15", setNum);
	//	}
}
