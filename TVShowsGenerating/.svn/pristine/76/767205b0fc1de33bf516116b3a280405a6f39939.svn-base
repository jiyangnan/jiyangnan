...
public class TwitterChannelQueryDay {
	...
	public void operFromTemp(String time,int setNum,String taday,String yesTaday){
		...
		String channelJudge = "select * from ufeng_twitter_tops_class_count_day where update_time= '"+taday+"'";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			+ "join wk_twitter_data_channel_date_day_new_news as d "
			+ "on c.code=d.channel  join  wk_twitter_data_channel_date_day_new_news as f on f.datetime='"+yesTaday+"'and "
			...
			DBUtil.addListMapForClassCount(channelMapList, "ufeng_twitter_tops_class_count_day");
			System.out.println("插入ufeng_twitter_tops_class_count_day表大小为："+channelMapList.size());
		}
