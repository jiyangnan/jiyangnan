...
public class ChannelQueryTwitter {
	...
	public void oper(String time,int setNum){
		...
		String sqlBlog = "SELECT CHANNEL from wk_twitter_data_channel_date where DATETIME = '"+time+"'";
		...
		if(CollectionUtils.isEmpty(list)){
			//生成channel中间表
			String sql = "SELECT e.CHANNEL,sum(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`,sum(`e`.`CONCERNSCOUNT`) AS `CONCERNSCOUNT`,sum(`e`.`COLLECTCOUNT`) AS `COLLECTCOUNT`,sum(`e`.`FORWARDCOUNT`) AS `FORWARDCOUNT`,count(`e`.`URL`) AS `CITEMS`,e.DATETIME from  wk_twitter_data_paste e WHERE DATETIME='"+time+"' and UPLOADTIME>= DATE_ADD(NOW(),INTERVAL "+setNum+" DAY) GROUP BY CHANNEL";
			...
			DBUtil.addListMap(mapList, "wk_twitter_data_channel_date");

			//生成channel表
			String channelSql = "SELECT  a.*,a.id as top_id, d.REVIEWCOUNT as review_count ,d.CONCERNSCOUNT as concerns_count,d.COLLECTCOUNT as collect_count , d.FORWARDCOUNT as forword_count ,d.CITEMS as citems_count,d.DATETIME as update_time FROM ufeng_tops as a JOIN ufeng_tops_to_channel as b on a.id= b.tops_id JOIN ufeng_tops_channel as c on c.id = b.channel_id JOIN wk_twitter_data_channel_date as d on c.code=d.channel and d.DATETIME='"+time+"'";

			List<Map<String,Object>> channelMapList = null;
			try {
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());

				System.out.println("插入ufeng_twitter_tops_class_count表的大小"+channelMapList.size());
			} catch (SQLException e) {
				e.printStackTrace();
			}

			DBUtil.addListMapForClassCount(channelMapList, "ufeng_twitter_tops_class_count");

			//生成Tv表
			new TvQueryTwitter().oper(time);
		}else{
	...
	public void operNew(String time,int setNum){
		...
		if(CollectionUtils.isEmpty(list)){
			if(time.equals("2015-08-23")){
				String sql = "SELECT e.CHANNEL,sum(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`,sum(`e`.`CONCERNSCOUNT`) AS `CONCERNSCOUNT`,sum(`e`.`COLLECTCOUNT`) AS `COLLECTCOUNT`,sum(`e`.`FORWARDCOUNT`) AS `FORWARDCOUNT`,count(`e`.`URL`) AS `CITEMS`,e.DATETIME from  wk_twitter_data_2015_35 e WHERE DATETIME='"+time+"' and UPLOADTIME>= DATE_ADD(NOW(),INTERVAL "+setNum+" DAY) GROUP BY CHANNEL";
				...
				System.out.println("查询表为："+"wk_twitter_data_"+paramYear+"_"+paramWeek+"    传入time为："+time);
				String sql = "SELECT e.CHANNEL,sum(`e`.`REVIEWCOUNT`) AS `REVIEWCOUNT`,sum(`e`.`CONCERNSCOUNT`) AS `CONCERNSCOUNT`,sum(`e`.`COLLECTCOUNT`) AS `COLLECTCOUNT`,sum(`e`.`FORWARDCOUNT`) AS `FORWARDCOUNT`,count(`e`.`URL`) AS `CITEMS`,e.DATETIME from  wk_twitter_data_"+paramYear+"_"+paramWeek+" e WHERE DATETIME='"+time+"' and UPLOADTIME>= DATE_ADD(NOW(),INTERVAL "+setNum+" DAY) GROUP BY CHANNEL";
				List<Map<String,Object>> mapList = null;
				try {
					mapList = DBHelper.getQueryRunner().query(sql, new MapListHandler());
	...
	public static void main(String[] args) {
		...
		while(!taday.equals("2015-08-23")){
			System.out.println("&&&&&&&&&&&&&&&Twitterß&&&&&&&&&&&&&&&&&");
			System.out.println("&&&&&&&&&&&&&&&8月23日以前版本&&&&&&&&&&&&&&&&&");
			System.out.println("执行今日日期："+taday);
			try {
				Calendar cal2 = Calendar.getInstance();
				Calendar cal3 = Calendar.getInstance();
				new ChannelQueryTwitter().oper(taday,setNum);
				...
				new ChannelQueryTwitter().oper(yesterday,setNum);
				new ChannelZengLiangTwitter().oper(taday,yesterday);
		...
		while(!yesterday.equals(currentTime)){
			...
			try {
				Calendar cal2 = Calendar.getInstance();
				Calendar cal3 = Calendar.getInstance();
				new ChannelQueryTwitter().operNew(taday,setNum);
				Date myDate = sdf.parse(taday);
				cal.setTime(myDate);
				cal2.setTime(cal.getTime());
				cal2.add(Calendar.DATE, -1);
				...
				cal.add(Calendar.DATE, +1);
				System.out.println("执行昨日日期："+yesterday);
				new ChannelQueryTwitter().operNew(yesterday,setNum);
				new ChannelZengLiangTwitter().oper(taday,yesterday);
				System.out.println("即将处理数据日期为："+cal3.getTime());
				System.out.println("taday:"+taday+"  ;  yesterday:"+yesterday);
				taday = sdf.format(cal.getTime());
