...
public class TwitterChannelQueryMonth {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				+ "join wk_twitter_data_channel_date_month as d "
				+ "on c.code=d.channel  join  wk_twitter_data_channel_date_month as f on f.MONTHNUM='"+lastLastMonthStr+"'and "
				...
				DBUtil.addListMapForClassCount(channelMapList, "ufeng_twitter_tops_class_count_month");
