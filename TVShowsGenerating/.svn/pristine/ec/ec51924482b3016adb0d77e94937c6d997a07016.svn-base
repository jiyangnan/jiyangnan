...
public class YtbTVQueryWeek {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
		String channelJudge = "SELECT  week from ufeng_ytb_tops_tv_count_week where week='"+WeekNow+"' limit 0,10";
		...
		if(judgeMapList!=null && judgeMapList.size()>1){
			...
			String channelSql = "SELECT a.*,a.id as top_id,b.week as week,"
					...
					+ "LEFT JOIN ufeng_ytb_tops_class_count_week as b "
					+ "ON a.id = b.pid WHERE a.level='1' and b.week ='"+WeekNow+"' GROUP BY id";
			
			List<Map<String,Object>> channelMapList = null;
			
				channelMapList = DBHelper.getQueryRunner().query(channelSql, new MapListHandler());

				System.out.println("插入ufeng_ytb_tops_tv_count_week表大小为："+channelMapList.size());
			

			DBUtil.addListMapForTVCount(channelMapList, "ufeng_ytb_tops_tv_count_week");
			System.out.println("插入ufeng_ytb_tops_tv_count_week表大小为："+channelMapList.size());
		}
...
//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
//		Date date = new Date();
//		new YtbTVQueryWeek().operFromTemp("2015-09-15", setNum);
//	}
}
