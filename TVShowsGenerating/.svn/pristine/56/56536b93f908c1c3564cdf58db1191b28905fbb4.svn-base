...
public class YtbChannelQueryMonth {
	...
	public void operFromTemp(String time,int setNum){
		...
		try {
			String channelJudge = "SELECT  month from ufeng_ytb_tops_class_count_month where month='"+lastMonthStr+"' limit 0,10";
			...
			if(judgeMapList!=null && judgeMapList.size()>1){
				...
				"ufeng_tops_channel as c on c.id = b.channel_id join wk_ytb_data_channel_date_month as d " +
				" on c.code=d.channel join wk_ytb_data_channel_date_month as f on f.MONTHNUM='"+lastLastMonthStr+"'" +
				...
				DBUtil.addListMapForClassCount(channelMapList, "ufeng_ytb_tops_class_count_month");
				System.out.println("插入ufeng_ytb_tops_class_count_month表大小为："+channelMapList.size());
